...
_Tugas Daily Project 4 — Rekayasa Kebutuhan (Semester 6)_
...
> ⚠️ Pastikan berkas `Data Alumni.xlsx` berada di folder root proyek (satu level di atas `backend/`)
...
Kredensial login disembunyikan menggunakan sistem **Environment Variables** (`ADMIN_USERNAME` & `ADMIN_PASSWORD`) demi keamanan repositori publik.
Anda harus mengonfigurasinya melalui pengaturan Environment Vars di _dashboard_ hosting Anda (misal: panel pengaturan Render.com).
...
├── 📄 Data Alumni.xlsx        # Data sumber alumni (tidak di-commit)
│
├── 📁 backend/
